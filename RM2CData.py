...
RM2C.exe rom="romname.z64" editor=False levels=[] actors=[] Append=[(rom,areaoffset,editor),...] WaterOnly=0 ObjectOnly=0 MusicOnly=0 MusicExtend=0 Text=0 Misc=0 Textures=0 Inherit=0 Upscale=0 Title=0 Sound=0 Objects=0

If you are on UNIX Bash, use:
./RM2C.exe instead of RM2C.exe

Example 1 (Export all Levels in a RM rom):
RM2C.exe rom="baserom.z64" levels=all

Example 2 (all actor models in BoB, lvl id 9):
RM2C.exe rom="ASA.z64" editor=True levels=[9] actors=all ObjectOnly=1

Example 3 (Export all BoB in a RM rom with a second area from another rom):
RM2C.exe rom="baserom.z64" levels=all Append=[("rom2.z64",1,True)]

Arguments with equals sign are shown in default state, do not put commas between args.
- Rom is the fullpath to the rom file.
- Levels accept any list argument or "all" for all levels, or "allplus" if the ROM implements levels whose id are not in the original ROM.
- Editor must be set to True if the rom was made using SM64 Editor or an old version of ROM Manager.
    You can also try to set Editor to True when the rom isn't exporting properly (even though it was made using ROM Manager)
- Actors will accept either a list of groups, a string for a group (see decomp group folders e.g. common0, group1 etc.) the string 'all' for all models,
    or the string 'new' for only models without a known label, or 'old' for only known original models.
- The "Only" options are to only export certain things either to deal with specific updates or updates to RM2C itself.
    Only use one at a time. An only option will not maintain other data.
    Do not use Append with MusicOnly, it will have no effect.
- MusicExtend is for when you want to add in your custom music on top of the original tracks.
    Set it to the amount you want to offset your tracks by (0x23 for vanilla).
- Objects will export behaviors and object collision.
    Possible args are 'all' for all behaviors used, 'new' for ones without a known label,
    or you can pass a singular or list of regex matches e.g. ['[0-9]','koopa'].
- Textures will export the equivalent of the /textures/ folder in decomp.
- Inherit is a file management arg for when dealing with multiple roms.
    Normal behavior is to clear level folder each time, inherit prevents this.
...
- Sound will export instrument bank and sound sample data.
    It does not seem to work with custom samples well.
- Upscale is an option to use ESRGAN ai upscaling to increase texture size.
    The upscaled textures will generate #ifdefs in each model file for non N64 targeting to compile them instead of the original textures.
- Append is for when you want to combine multiple roms.
    The appended roms will be use the levels of the original rom, but use the areas of the appended rom with an offset.
